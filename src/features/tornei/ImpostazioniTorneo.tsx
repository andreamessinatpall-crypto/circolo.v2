...
export default function ImpostazioniTorneo({ torneo }: { torneo: Torneo }) {
  ...
  return (
    <details className="card" style={{ marginTop: 16 }}>
      <summary style={{ cursor: 'pointer', fontWeight: 700 }}>⚙️ Modifica regole del torneo</summary>

      <p className="sub mt-2">
        Sport: <strong>{SPORT_LABEL[torneo.sport] ?? torneo.sport}</strong> · Formato fisso. Per
        cambiare sport crea un nuovo torneo.
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-2">
        ...
        {msg && <p className={`mt-4 ${msg.tipo === 'ok' ? classiOk : classiErrore}`}>{msg.testo}</p>}

        <button type="submit" className="btn mt-4" disabled={isSubmitting || !isDirty}>
